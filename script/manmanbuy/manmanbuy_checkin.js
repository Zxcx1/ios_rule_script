...
const options = {
  ...
  headers: {
    ...
    "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 18_7 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 - mmbWebBrowse - ios",
    "Cookie": cookie
  },
  body: body
};
  ...
  try {
    ...
    if (obj.ok === 1 || obj.code === 2000) {
      ...
      const msg =
        ...
        `💎 当前积分：${r.points}\n` +
        `🪙 金币：${r.coin}\n` +
        `📌 今日已签到：${r.signByToday ? "是" : "否"}`;

      magicJS.notify("🎉 签到成功", "", msg);
    } else {
