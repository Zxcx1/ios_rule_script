...
// 抓取 / 使用 的键名
const COOKIE_KEY = 'manmanbuy_app_cookie';
...
// 抓取用的正则
const SIGN_REQUEST_REGEX = /https?:\/\/basic-ucenter\.manmanbuy\.com\/user\/sign\/info/;

async function Main() {
  // 重写阶段：抓取 Cookie + Body
  if (magicJS.isRequest) {
    if (SIGN_REQUEST_REGEX.test(magicJS.request.url)) {
      try {
        const cookie = magicJS.request.headers['Cookie'] || magicJS.request.headers['cookie'] || '';
        const body = magicJS.request.body || '';

        if (!cookie || !body) {
          magicJS.log('抓到 sign 接口，但 Cookie 或 Body 为空，不保存。');
        } else {
          const oldCookie = magicJS.read(COOKIE_KEY);
          const oldBody = magicJS.read(BODY_KEY);

          if (cookie !== oldCookie || body !== oldBody) {
            magicJS.write(COOKIE_KEY, cookie);
            magicJS.write(BODY_KEY, body);
            magicJS.log('获取 App 签到参数成功，已更新本地存储。');
            magicJS.notify('🎈慢慢买参数获取成功', '', '已保存 Cookie 和 Body，可用于自动签到');
          } else {
            magicJS.log('App 签到参数无变化，无需更新。');
          }
        }
      } catch (e) {
        magicJS.log(`获取 App 签到参数失败：${e}`);
        magicJS.notify('❌慢慢买参数获取失败', '', String(e));
      }
    }
    magicJS.done();
    return;
  ...
  // 定时任务阶段：自动签到
  const cookie = magicJS.read(COOKIE_KEY);
  const body = magicJS.read(BODY_KEY);

  if (!cookie || !body) {
    magicJS.notify(
      '❌慢慢买签到失败',
      '',
      '未找到 Cookie 或 Body，请先在开启重写的情况下手动签到一次'
    );
    magicJS.done();
    return;
  }
  ...
  magicJS.post(options, (err, resp, data) => {
    if (err) {
      magicJS.log(`签到失败，HTTP 异常：${err}`);
      magicJS.notify('❌慢慢买签到失败', '', '网络异常：' + err);
      magicJS.done();
      return;
    }

    try {
      // 有些环境可能返回 JSONP，这里先简单去掉可能的 jsoncallback 包裹
      if (typeof data === 'string' && data.trim().startsWith('?(')) {
        data = data.trim().replace(/^\?`\(/, ^\prime^\prime).replace(/\)`;?$/, '');
      }
      ...
      if (obj.ok === 1 || obj.code === 2000) {
        ...
        magicJS.log(`签到成功：${bodyText.replace(/\n/g, ' | ')}`);
        magicJS.notify('🎉慢慢买签到成功', '', bodyText);
      } else {
        const msg = obj.msg || '未知错误';
        magicJS.log(`签到失败，接口返回：${data}`);
        magicJS.notify('❌慢慢买签到失败', '', msg);
      }
    } catch (e) {
      magicJS.log(`签到失败，解析异常：${e}，原始响应：${data}`);
      magicJS.notify('❌慢慢买签到失败', '', '解析异常：' + e);
    }
