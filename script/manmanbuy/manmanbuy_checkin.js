  ...
  try {
    if (SIGN_REGEX.test(magicJS.request.url)) {
      ...
      if (cookie && body) {
        ...
        magicJS.notify("🎉 慢慢买参数获取成功", "", "已保存 Cookie 和 Body");
      }
    }
  } catch (e) {
    magicJS.notify("❌ 参数获取失败", "", String(e));
  }
  magicJS.done();
  return;
}
...
if (!cookie || !body) {
  magicJS.notify("❌ 无法签到", "", "请先手动签到一次以抓取参数");
  magicJS.done();
  return;
}
...
function makeTrendChart(arr) {
  ...
  return arr
    .map(v => {
      const pos = Math.round(((v - min) / range) * 8);
      return `${v} ┤${" ".repeat(pos)}●`;
    })
    .join("\n");
}
...
magicJS.post(options, (err, resp, data) => {
  ...
  try {
    ...
    // ========== 今日已签到 ==========
    if (obj.code === 4001 || (obj.msg && obj.msg.includes("已签到"))) {
    ...
    if (obj.ok === 1 && obj.code === 2000) {
      ...
      // ========== 金币变化（如果接口返回 coin）==========
      const coin = r.coin || 0;
      const lastCoins = magicJS.read(LAST_COINS_KEY) || 0;
      const diffCoins = coin - lastCoins;
      magicJS.write(LAST_COINS_KEY, coin);
      ...
      if (coinsHistory[coinsHistory.length - 1] !== coin) {
        coinsHistory.push(coin);
      }
      ...
      const coinsChart = makeTrendChart(coinsHistory);

      // ========== 箭头 ==========
      const arrowP = diffPoints > 0 ? "📈" : diffPoints < 0 ? "📉" : "➡️";
      const arrowC = diffCoins > 0 ? "🪙📈" : diffCoins < 0 ? "🪙📉" : "🪙➡️";

      // ========== 通知 ==========
      const msg =
        `🎉 ${r.title}\n` +
        `💎 积分：${r.point}（${arrowP} ${diffPoints >= 0 ? "+" : ""}${diffPoints}）\n` +
        `🪙 金币：${coin}（${arrowC} ${diffCoins >= 0 ? "+" : ""}${diffCoins}）\n\n` +
        `📊 积分趋势（最近 ${HISTORY_DAYS} 天）\n${pointsChart}\n\n` +
        `🪙 金币趋势（最近 ${HISTORY_DAYS} 天）\n${coinsChart}\n\n` +
        `🎯 ${r.description}`;
    ...
    magicJS.notify("❌ 解析异常", "", String(e));
  }

  magicJS.done();
});

/**************** MagicJS 内核（保持你原来的完整版本）****************/
